...
router.get('/overview', requireAnyAuth, (req, res) => {
  ...
  const last7 = db.prepare(`
    SELECT date(created_at) as day, SUM(views) as views, SUM(likes) as likes
    FROM content WHERE agent_id = ? AND created_at >= date('now', '-7 days')
    GROUP BY day ORDER BY day ASC
  `).all(id);

  const topContent = db.prepare(`
    SELECT id, title, type, views, likes, created_at
    FROM content WHERE agent_id = ? AND status='active'
    ORDER BY views DESC LIMIT 5
  `).all(id);

  const revenueEstimate = {
    subscriptions: (req.agent.plan === 'pro' ? 8.99 : req.agent.plan === 'studio' ? 48.99 : 0),
    ads: parseFloat((totalViews * 0.002).toFixed(2)),
    licenses: parseFloat((totalLikes * 0.05).toFixed(2)),
    total: 0
  };
  revenueEstimate.total = parseFloat((revenueEstimate.subscriptions + revenueEstimate.ads + revenueEstimate.licenses).toFixed(2));
  ...
  res.json({
    ...
    subscribers,
    engagement_rate: totalViews > 0 ? parseFloat(((totalLikes / totalViews) * 100).toFixed(2)) : 0,
    revenue_estimate: revenueEstimate,
    views_last_7_days: last7,
...
router.get('/content/:id', requireAnyAuth, (req, res) => {
  ...
  if (!content) return res.status(404).json({ error: 'Content not found or not yours' });
  ...
  res.json({
    ...
    revenue_estimate: parseFloat((content.views * 0.002).toFixed(2))
  });
});
